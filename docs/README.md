**Infuse-TS**

***

# Infuse-TS

## Enumerations

- [Scope](enumerations/Scope.md)

## Classes

- [InvalidDataError](classes/InvalidDataError.md)
- [InvalidOperationError](classes/InvalidOperationError.md)
- [NotRegisteredError](classes/NotRegisteredError.md)
- [NotSupportedError](classes/NotSupportedError.md)
- [Container](classes/Container.md)
- [RequestContext](classes/RequestContext.md)

## Interfaces

- [IContainer](interfaces/IContainer.md)
- [IValueRegistration](interfaces/IValueRegistration.md)
- [IReferenceRegistration](interfaces/IReferenceRegistration.md)

## Type Aliases

- [AbstractComponent](type-aliases/AbstractComponent.md)
- [Component](type-aliases/Component.md)
- [ComponentType](type-aliases/ComponentType.md)
- [ComponentArgs](type-aliases/ComponentArgs.md)
- [MethodHandle](type-aliases/MethodHandle.md)
- [ClassDecoration](type-aliases/ClassDecoration.md)
- [MethodDecoration](type-aliases/MethodDecoration.md)
- [FieldDecoration](type-aliases/FieldDecoration.md)
- [Identifier](type-aliases/Identifier.md)
- [Collection](type-aliases/Collection.md)
- [Factory](type-aliases/Factory.md)
- [Dependency](type-aliases/Dependency.md)
- [Optionally](type-aliases/Optionally.md)
- [Dependencies](type-aliases/Dependencies.md)
- [DependencyItem](type-aliases/DependencyItem.md)
- [DependencyType](type-aliases/DependencyType.md)

## Functions

- [Inject](functions/Inject.md)
- [InjectableAs](functions/InjectableAs.md)
- [Injectable](functions/Injectable.md)
- [Optional](functions/Optional.md)
- [isComponent](functions/isComponent.md)
- [isIdentifier](functions/isIdentifier.md)
- [isCollection](functions/isCollection.md)
